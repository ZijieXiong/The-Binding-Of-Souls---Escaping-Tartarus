...
//	}
//}
if (global.currLevel > 0 and global.currLevel <= 3) {
	audio_sound_gain(snd_jungle, 0.2, 0);
	if (!audio_is_playing(snd_jungle)) {
	audio_play_sound(snd_jungle, 0, true);
	}
}
if (global.currLevel >= 4 and global.currLevel <= 6) {
	audio_sound_gain(snd_jungle, 0, 2000);
	audio_stop_sound(snd_jungle);
	audio_sound_gain(snd_pyramid_bgm, 0.2, 0);
...
if (global.currLevel >= 7) {
	...
	audio_stop_sound(snd_pyramid_bgm);
	audio_sound_gain(snd_scifi, 0.2, 0);
