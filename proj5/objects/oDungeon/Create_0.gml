...
GenerateNewDungeon = function() {
	...
	tilemap_clear(layer_tilemap_get_id(layer_get_id("Tiles_1_1")), 0);
	with(obj_wall){
		instance_destroy();
	}
	with(oEnemiesParent){
	...
	for (var xx = 0; xx < _dungeonWidth; xx++) {
		for (var yy = 0; yy < _dungeonHeight; yy++) {
			...
			tilemap_set(layer_tilemap_get_id(layer_get_id("Tiles_1_1")), _tileInd, xx, yy);
		}
	}
	...
	for (var i = 0; i < ds_list_size(deadEnd); i++) {
	    ...
	    if (roomInd != reloadRoomInd && roomInd != chestRoomInd) {
	        if (random(1) <= 1) {
	            ...
	            var eliteEnemy = instance_create_layer((roomId.x1 + roomId.x2) / 2 * CELL_SIZE, (roomId.y1 + roomId.y2) / 2 * CELL_SIZE, "Dungeon", oSlimeAlpha);
...
CreateHallway = function(_x1, _y1, _x2, _y2, isNorthSouth) {
    ...
    if (isNorthSouth) {
		...
		for (var temp_x = _x1-1; temp_x <= _x2+1; temp_x++) {
			...
			var coverInstanceDown = instance_position(temp_x * CELL_SIZE, (_y2 + 1) * CELL_SIZE, obj_wall_wallcover);
